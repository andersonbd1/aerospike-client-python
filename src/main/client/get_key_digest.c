	...
	PyObject * py_ns, PyObject *py_set, PyObject * py_key)
{
	...
	if ( !PyStr_Check(py_set)  && !PyUnicode_Check(py_set) ) {
		PyErr_SetString(PyExc_TypeError, "Set should be a string or unicode");
		return NULL;
	}
	if ( !PyStr_Check(py_key)  && !PyUnicode_Check(py_key) && !PyInt_Check(py_key) && !PyLong_Check(py_key) && !PyByteArray_Check(py_key) ) {
